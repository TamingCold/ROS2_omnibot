...
class RobotControlNode(Node):
    def __init__(self, robot_name, other_robots):
    ...
    def get_robot_position(self, robot_name):
        try:
            # Lookup transformation from world frame ('odom' or 'map') to robot's frame ('base_link')
    ...
    def update_velocity(self):
        ...
        if count > 0:
            ...
            velocity_msg = Twist()
            velocity_msg.linear.x = 0.5 * distance_x  
            velocity_msg.linear.y = 0.5 * distance_y  
...
def main(args=None):
    ...
    # Pass the parameters to the control node
    control_node = RobotControlNode(robot_name, other_robots)
